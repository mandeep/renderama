...
pub fn cornell_box_object_scene(width: Option<usize>, height: Option<usize>) -> Scene {
    ...
    world.add(TransformedMesh::new(Vec3A::new(110.0, -25.0, 140.0), Vec3A::new(0.0, 180.0, 0.0), 750.0, bunny).into());
    ...
    world.add(TransformedMesh::new(Vec3A::new(273.0, 180.0, 530.0), Vec3A::new(-90.0, 180.0, 0.0), 24.0, buddha).into());

    let bvh = BVH::new(&mut world.objects, 0.0, 1.0);

    let light_shape = Plane::new(Axis::XZ, Bounds2D::new(213.0..343.0, 227.0..332.0), 554.0, white_id);

    Scene::new(String::from("Cornell Box with Multiple Objects"), bvh, materials, camera, vec![Light::new(light_shape.into(), Vec3A::new(25.0, 18.0, 10.0))], None, false)
}