...
pub fn cornell_box_uv_scene(width: Option<usize>, height: Option<usize>) -> Scene {
    ...
    let roughness = 0.0;
    let texture_id = mat!(materials, Diffuse::new(ImageTexture::new("extras/textures/uv_checker.jpg", 1.0).into(), roughness));
    ...
    objects.push(Plane::new(Axis::XZ, Bounds2D::new(213.0..343.0, 227.0..332.0), 554.70, light_material).into_reversed());
    ...
    objects.push(TransformedMesh::new(Vec3A::new(130.0, 0.0, 65.0), Vec3A::new(0.0, -18.0, 0.0), Vec3A::splat(1.0), Rectangle::new(p0, p1, texture_id).into()).into());
    objects.push(TransformedMesh::new(Vec3A::new(265.0, 0.0, 295.0), Vec3A::new(0.0, 15.0, 0.0), Vec3A::splat(1.0), Rectangle::new(p0, p2, texture_id).into()).into());

    let light_shape = Plane::new(Axis::XZ, Bounds2D::new(213.0..343.0, 227.0..332.0), 554.70, light_material);
