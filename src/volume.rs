    ...
    fn hit(&self, ray: &Ray, t_min: f32, t_max: f32) -> Option<HitRecord> {
        if let Some(mut hit1) = self.boundary.hit(ray, f32::MIN, f32::MAX) {
            if let Some(mut hit2) = self.boundary.hit(ray, hit1.parameter + 0.0001, f32::MAX) {
                ...
                if hit1.parameter >= hit2.parameter { return None };
                if hit1.parameter < 0.0 { hit1.parameter = 0.0 };
                ...
                if hit_distance < distance_inside_boundary {
                    let root = hit1.parameter + hit_distance / ray.direction.norm();
                    let point = ray.point_at_parameter(root);
                    let normal = Vector3::new(1.0, 0.0, 0.0);
                    return Some(HitRecord::new(root, 0.0, 0.0, point, normal, self.material.clone()));
                }
            }
        }
        return None;
