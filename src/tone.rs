...
/// Apply one of the tone mapping functions to the given rgb
pub fn apply_tonemap<F>(rgb: Vec3A, map_fn: F) -> Vec3A where F: Fn(f32) -> f32 {
    const LUM_WEIGHTS: Vec3A = Vec3A::new(0.2126, 0.7152, 0.0722);
    let luminance = rgb.dot(LUM_WEIGHTS);
    if luminance < 1e-6 {
        return rgb;
    }

    let mapped = map_fn(luminance);
