...
pub fn hyperion_scene(width: Option<usize>, height: Option<usize>) -> Scene {
    ...
    let floor_id = textures.add_texture(Color::new(0.63, 0.61, 0.59));
    let glass_id = textures.add_texture(Color::new(1.0, 1.0, 1.0));
    let metal_id = textures.add_texture(Color::new(0.93, 0.93, 0.93));
    let dark_metal_id = textures.add_texture(Color::new(0.757, 0.729, 0.694));
    let platform_id = textures.add_texture(Color::new(0.76, 0.74, 0.72));
    let orange_id = textures.add_texture(orange_color);
    let orange_rough_id = textures.add_texture(orange_bright_color);
    let marble_vol_id = textures.add_texture(Color::new(0.60, 0.71, 0.49));
    let pingpong_id = textures.add_texture(Color::new(0.93, 0.89, 0.85));
    let white_id = textures.add_texture(Color::new(1.0, 1.0, 1.0));

    let floor_idx = materials.add_material(Diffuse::new(floor_id, 0.0));
    let glass_idx = materials.add_material(Refractive::new(glass_id, 1.5));
    let metal_idx = materials.add_material(Reflective::new(metal_id, 0.2));
    ...
    let meshes = load_obj("extras/models/pokeball.obj", &mut materials, &mut textures, None);
    for mesh in meshes {
