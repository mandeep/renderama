...
pub fn spheres_in_box_scene(width: Option<usize>, height: Option<usize>, rng: &mut Pcg64Mcg) -> Scene {
    ...
    let white = mat!(materials, Diffuse::new(0.0));
    let red_id = tex!(textures, Color::new(1.0, 0.10, 0.20));
    let red = mat!(materials, Diffuse::new(0.0));
    let light_id = tex!(textures, Color::new(7.0, 7.0, 7.0));
    let big_light = mat!(materials, Emissive::new());
    let snow_id = tex!(textures, Color::new(0.48, 0.83, 0.53));
    let snow = mat!(materials, Diffuse::new(0.0));
    ...
    for i in 0..number_of_boxes {
        for j in 0..number_of_boxes {
            ...
            objects.push_into(Rectangle::new(p0, p1, snow, snow_id));
        }
    }

    objects.push_into(Plane::new(Axis::XZ, Bounds2D::new(123.0..423.0, 147.0..412.0), 554.0, big_light, light_id).into_reversed());

    let sphere = Sphere::new(Vec3A::new(0.0, 0.0, 0.0), 1.0, red, red_id);
    ...
    objects.push_into(Sphere::new(Vec3A::new(260.0, 150.0, 45.0), 50.0, refr_idx, refr_id));

    let refl_id = tex!(textures, Color::new(0.8, 0.8, 0.9));
    let refl_idx = mat!(materials, Reflective::new(0.0));
    objects.push_into(Sphere::new(Vec3A::new(0.0, 150.0, 145.0), 50.0, refl_idx, refl_id));

    let boundary = Sphere::new(Vec3A::new(360.0, 150.0, 145.0), 70.0, refr_idx, refr_id);
    ...
    objects.push_into(Volume::new(0.2, cloned_boundary, vol_idx, vol_id));

    let fog = Sphere::new(Vec3A::new(0.0, 0.0, 0.0), 5000.0, refr_idx, refr_id);

    let fog_id = tex!(textures, Color::new(1.0, 1.0, 1.0));
    let fog_idx = mat!(materials, Volumetric::new());
    objects.push_into(Volume::new(0.0001, fog, fog_idx, fog_id));
    ...
    let topo_idx = mat!(materials, Diffuse::new(0.0));
    objects.push_into(Sphere::new(Vec3A::new(400.0, 200.0, 400.0), 100.0, topo_idx, topo_id));

    let marble_id = tex!(textures, ImageTexture::new("extras/textures/marble.jpg", Vec2::splat(2.0)));
    let marble = mat!(materials, Diffuse::new(0.0));
    objects.push_into(Sphere::new(Vec3A::new(220.0, 280.0, 300.0), 80.0, marble, marble_id));
    ...
    for _ in 0..number_of_spheres {
        ...
        let sphere = Sphere::new(center, 10.0, white, white_id);
    ...
    let light_shape = Plane::new(Axis::XZ, Bounds2D::new(123.0..423.0, 147.0..412.0), 554.0, white, white_id);
