...
pub fn spheres_in_box_scene(width: Option<usize>, height: Option<usize>, rng: &mut Pcg64Mcg) -> Scene {
    ...
    let white = mat!(materials, Diffuse::new(Color::new(0.73, 0.73, 0.73), 0.0));
    let red = mat!(materials, Diffuse::new(Color::new(1.0, 0.10, 0.20), 0.0));
    let big_light = mat!(materials, Emissive::new(Color::new(7.0, 7.0, 7.0)));
    let snow = mat!(materials, Diffuse::new(Color::new(0.48, 0.83, 0.53), 0.0));
    ...
    let refl_idx = mat!(materials, Reflective::new(Color::new(0.8, 0.8, 0.9), 0.0));
    ...
    let vol_idx = mat!(materials, Volumetric::new(Color::new(0.2, 0.4, 0.9)));
    objects.push_into(Volume::new(0.2, cloned_boundary, vol_idx));

    let fog = Sphere::new(Vec3A::new(0.0, 0.0, 0.0), 5000.0, refr_idx);

    let fog_idx = mat!(materials, Volumetric::new(Color::new(1.0, 1.0, 1.0)));
    ...
    let topo_idx = mat!(materials, Diffuse::new(ImageTexture::new("extras/textures/world_topo_nasa.jpg", Vec2::splat(1.0)), 0.0));
    objects.push_into(Sphere::new(Vec3A::new(400.0, 200.0, 400.0), 100.0, topo_idx));

    let marble = mat!(materials, Diffuse::new(ImageTexture::new("extras/textures/marble.jpg", Vec2::splat(2.0)), 0.0));
